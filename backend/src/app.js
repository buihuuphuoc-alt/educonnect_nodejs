...
// ── Static files ──────────────────────────────────────────────────────
// Frontend (public/views) giờ nằm ở thư mục ../../frontend so với file này
const FRONTEND_DIR = path.join(__dirname, '..', '..', 'frontend');
app.use(express.static(path.join(FRONTEND_DIR, 'public')));
app.use('/uploads/materials', express.static(path.join(FRONTEND_DIR, 'public', 'uploads', 'materials')));

// ── Nunjucks template engine ──────────────────────────────────────────
const env = nunjucks.configure(path.join(FRONTEND_DIR, 'views'), {
  autoescape: true,
  express: app,
