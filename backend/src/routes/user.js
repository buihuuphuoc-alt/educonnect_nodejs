...
// ── Multer setup ──────────────────────────────────────────────────────
const uploadDir = path.join(__dirname, '../../../frontend/public/images/courses');
...
const materialUploadDir = path.join(__dirname, '../../../frontend/public/uploads/materials');
...
router.post('/chinh-sua-material/:mat_id', loginRequired, (req, res, next) => {
  uploadMaterial.single('material_file')(req, res, async (err) => {
    ...
    if (req.file) {
      // Xóa file cũ nếu có
      if (ok.old_url) {
        const oldPath = path.join(__dirname, '../../../frontend/public', ok.old_url);
        fs.unlink(oldPath, () => {});
      }
...
router.post('/xoa-material/:mat_id', loginRequired, async (req, res) => {
  ...
  if (!ok || ok.instructor_id !== req.session.user_id) return res.json({ success: false, message: 'Không có quyền.' });
  if (ok.url) {
    const filePath = path.join(__dirname, '../../../frontend/public', ok.url);
